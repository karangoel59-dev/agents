...
# Load environment variables from default .env and api_key.env if it exists
load_dotenv()
...
class OpenAIApiProxy():
    MODEL_API_VERSION_MAP = {
        "gpt-4.1": os.getenv("GPT_41_API_VERSION"),
        "gpt-4.1-2": os.getenv("GPT_41_2_API_VERSION"),
        "gpt-4.1-mini": os.getenv("GPT_41_MINI_API_VERSION"),
        "gpt-4.1-mini-2": os.getenv("GPT_41_MINI_2_API_VERSION"),
        "gpt-4.1-mini-standard": os.getenv("GPT_41_MINI_STANDARD_API_VERSION"),
        "gpt-4.1-nano": os.getenv("GPT_41_NANO_API_VERSION"),

        "gpt-4o": os.getenv("GPT_4O_API_VERSION"),
        "gpt-4o-mini": os.getenv("GPT_4O_MINI_API_VERSION"),
        "gpt-4o-standard": os.getenv("GPT_4O_STANDARD_API_VERSION"),

        "gpt-5-mini": os.getenv("GPT_5_MINI_API_VERSION"),
        "gpt-5-nano": os.getenv("GPT_5_NANO_API_VERSION"),

        "gpt-5.4-mini": os.getenv("GPT_54_MINI_API_VERSION"),
        "gpt-5.4-nano": os.getenv("GPT_54_NANO_API_VERSION"),

        "text-embedding-3-large": os.getenv("TEXT_EMBEDDING_3_LARGE_API_VERSION"),
    }

    @classmethod
    def get_api_version(cls, model_name):
        version = cls.MODEL_API_VERSION_MAP.get(model_name)
        if version:
            return version
    ...
    def call_embedding(self, model, text, use_official=None, **kwargs):
        if use_official == "azure":
            client = AzureOpenAI(
                azure_endpoint=os.getenv("AZURE_OPENAI_ENDPOINT"),
                api_key=os.getenv("AZURE_OPENAI_API_KEY"),
                api_version=self.get_api_version(model)
            )
    ...
    def call(self, model, messages, no_cache = False, overwrite_cache=False, tools=None, temperature=None, headers={}, use_official=None, **kwargs):
        ...
        # Handle Azure OpenAI API via official AzureOpenAI client
        if use_official == "azure":
            try:
                client = AzureOpenAI(
                    azure_endpoint=os.getenv("AZURE_OPENAI_ENDPOINT"),
                ...
                result = completion.model_dump()["choices"]
                ...
                logger.error(f"Error with Azure OpenAI API: {e}")
                raise
